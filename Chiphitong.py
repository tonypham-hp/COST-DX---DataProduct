...
class UnifiedCostVisualization:
    def __init__(self, base_path):
        ...
        self.month_names = ['Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 
    ...
    def get_month_folders(self, export_path):
        """Get list of month folders with fiscal year mapping"""
        month_folders = {}
        year_month_mapping = {}  # Store all calendar_month -> year mapping
        
        # First pass: collect all month-year data
        for folder in os.listdir(export_path):
            ...
            if os.path.isdir(folder_path):
                ...
                if match:
                    calendar_month = int(match.group(1))
                    year = int(match.group(2))
                    year_month_mapping[calendar_month] = year
        
        # Determine fiscal year start (Apr of which year)
        fiscal_start_year = None
        if 4 in year_month_mapping:
            fiscal_start_year = year_month_mapping[4]
        elif year_month_mapping:
            # If April not found, infer from other months
            for cal_month, yr in year_month_mapping.items():
                if cal_month >= 4:
                    fiscal_start_year = yr
                    break
                elif cal_month < 4:
                    fiscal_start_year = yr - 1
                    break
        
        # Second pass: assign fiscal months with correct years
        for folder in os.listdir(export_path):
            folder_path = os.path.join(export_path, folder)
            if os.path.isdir(folder_path):
                match = re.search(r'Tháng\s+(\d{1,2})\.(\d{4})', folder, re.IGNORECASE)
                if match:
                    ...
                    # Convert to fiscal month (Apr=1, ..., Mar=12)
                    if calendar_month >= 4:
                        fiscal_month = calendar_month - 3  # Apr=1, May=2, ..., Dec=9
                    else:
                        fiscal_month = calendar_month + 9  # Jan=10, Feb=11, Mar=12
                    
                    month_folders[fiscal_month] = {
                        'path': folder_path, 
                        'year': year,  # Keep actual year from folder name
                        'calendar_month': calendar_month
                    }
        
        return month_folders
    
    def get_month_label(self, fiscal_month, month_folders):
        """Get month label like 'Apr 2025', 'May 2025', etc."""
        if fiscal_month in month_folders:
            year = month_folders[fiscal_month]['year']
            month_name = self.month_names[fiscal_month - 1]
            return f"{month_name} {year}"
        else:
            # If month folder doesn't exist, return placeholder
            month_name = self.month_names[fiscal_month - 1]
            return f"{month_name} ----"
    ...
    def process_all_months(self):
        """Process all months in the fiscal year"""
        ...
        # Process fiscal months 1-12 (Apr-Mar)
        for fiscal_month in range(1, 13):
            if fiscal_month in month_folders:
                month_label = self.get_month_label(fiscal_month, month_folders)
                progress_text.text(f"📊 Processing {month_label}...")
                month_path = month_folders[fiscal_month]['path']
                excel_file = self.find_excel_in_month_folder(month_path)
                
                if excel_file:
                    result = self.process_excel_file(excel_file)
                    if result:
                        self.months_data[fiscal_month] = result
                    else:
                        self.months_data[fiscal_month] = {team: {'BM': 0, 'TH': 0, 'PM_CM': 0} for team in self.teams}
                else:
                    self.months_data[fiscal_month] = {team: {'BM': 0, 'TH': 0, 'PM_CM': 0} for team in self.teams}
            else:
                self.months_data[fiscal_month] = {team: {'BM': 0, 'TH': 0, 'PM_CM': 0} for team in self.teams}
        
        # Store month_folders for label generation
    ...
    def create_visualization(self, selected_team):
        ...
        fiscal_months = list(range(1, 13))  # 1=Apr, 2=May, ..., 12=Mar
        months_labels = [self.get_month_label(fm, self.month_folders) for fm in fiscal_months]
        
        bm_values = [self.months_data.get(m, {}).get(selected_team, {}).get('BM', 0) for m in fiscal_months]
        th_values = [self.months_data.get(m, {}).get(selected_team, {}).get('TH', 0) for m in fiscal_months]
        pm_cm_values = [self.months_data.get(m, {}).get(selected_team, {}).get('PM_CM', 0) for m in fiscal_months]
        
        # Title based on selection
        if selected_team == 'All':
            title1 = '📊 Total Cost by 3 Categories (Fiscal Year)'
            title2 = '📈 12-Month Total Cost by Category'
        else:
            title1 = f'📊 {selected_team} Cost by 3 Categories (Fiscal Year)'
        ...
        fig.update_xaxes(title_text="", row=2, col=1, tickangle=-45, tickfont=dict(size=13), title_font=dict(size=16))
...
def process_data(base_path, selected_team):
    ...
    try:
        ...
        data_table = []
        for fiscal_month in range(1, 13):
            month_label = viz.get_month_label(fiscal_month, viz.month_folders)
            data = viz.months_data.get(fiscal_month, {}).get(selected_team, {})
            data_table.append({
                'Month': month_label,
